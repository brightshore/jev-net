...
// What a broken trimmed path usually looks like is not a wrong value but an EXCEPTION nobody planned for — a
// MissingMethodException, a KeyNotFoundException from an answer that never got decoded. Left alone that is a
// bare stack trace (and under AOT, not much of one) with no check named. So every section runs through here:
// an escape becomes a named FAIL carrying the exception, and the sections after it still run.
async Task Section(string name, Func<Task> body)
{
    ...
    {
        Check($"{name}: completed without an unexpected exception", false, $"{error.GetType().Name}: {error.Message}");
    }
}
...
JsonContent state = "fallback state"; // replaced below; a section that throws must not strand the later ones
await Section("plain path", async () =>
{
    state = JsonContent.From(new Ticket("Charged twice", ["billing"]), SmokeJson.Default.Ticket);
...
await Section("answers by property name", async () =>
{
...
await Section("JsonTypeInfo response", async () =>
{
...
await Section("429 mapping", async () =>
{
    try
    {
...
await Section("malformed response", async () =>
{
    try
    {
